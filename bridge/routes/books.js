...
function createBookRouter(claudeManager) {
    ...
    try {
      ...
      (async () => {
        try {
          ...
          if (bookJobs.get(jobId)?.status === 'cancelled') return;
  ...
  router.delete('/:bookId/chat-result/:jobId', httpAuth, (req, res) => {
    const job = bookJobs.get(req.params.jobId);
    if (!job) {
      res.json({ ok: true, status: 'unknown' });
      return;
    }
    bookJobs.set(req.params.jobId, { ...job, status: 'cancelled', at: Date.now() });
