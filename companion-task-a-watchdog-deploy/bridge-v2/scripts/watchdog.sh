...
if [[ "$count" -gt 1 ]]; then
  if [[ -z "${BRIDGE_TOKEN:-}" ]]; then
    log "[watchdog] duplicate Claude count=$count, but BRIDGE_TOKEN is empty; cannot call restart endpoint"
    exit 1
  fi
  log "[watchdog] duplicate Claude count=$count; calling restart endpoint"
  curl -fsS --max-time 90 \
    -X POST \
    -H "Authorization: Bearer ${BRIDGE_TOKEN}" \
    -H "Content-Type: application/json" \
    -d '{"reason":"watchdog_duplicate_processes"}' \
