...
const insertMessageStmt = db.prepare(`
  INSERT INTO messages (session_id, conversation_id, source, role, content, model, ts, meta, keepalive_consumed)
  VALUES (@session_id, @conversation_id, @source, @role, @content, @model, @ts, @meta, @keepalive_consumed)
`);

const recentMessagesStmt = db.prepare(`
  SELECT id, session_id, conversation_id, source, role, content, model, ts, meta, keepalive_consumed
  FROM messages
...
function insertMessage({ sessionId, conversationId = 'default', source = 'chat', role, content, model = null, meta = null, keepaliveConsumed = 0 }) {
...
function getPendingKeepalive(sessionId, conversationId = 'default') {
  return db.prepare(`
    SELECT id, session_id, conversation_id, source, role, content, model, ts, meta, keepalive_consumed
    FROM messages
    WHERE session_id = ? AND conversation_id = ? AND source = 'keepalive' AND keepalive_consumed = 0
    ORDER BY ts ASC
...
function getLastKeepalive(sessionId, conversationId = 'default') {
  return db.prepare(`
    SELECT id, content, ts
    FROM messages
    WHERE session_id = ? AND conversation_id = ? AND source = 'keepalive'
    ORDER BY ts DESC
    LIMIT 1
  `).get(sessionId, normalizeConversationId(conversationId));
}
