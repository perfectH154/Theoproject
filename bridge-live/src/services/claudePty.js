...
const FIXED_CLAUDE_MODEL = 'claude-opus-4-6';
...
function transcriptPathForSession(workdir, sessionId) {
  if (!sessionId) return null;
  return path.join(transcriptDirForWorkdir(workdir), `${sessionId}.jsonl`);
}

function parsePermissionInputPreview(inputPreview) {
  try {
    const parsed = JSON.parse(String(inputPreview || '{}'));
...
function isUploadReadPermission(payload) {
  if (String(payload?.tool_name || '') !== 'Read') return false;
  const input = parsePermissionInputPreview(payload?.input_preview);
...
function extractThinkingFromTranscriptLine(line) {
  let event;
  try {
  ...
  for (const block of blocks) {
    ...
    const text = String(block.thinking || block.text || block.content || '').trim();
    out.push({
      text,
      placeholder: !text && Boolean(block.signature),
      hidden: !text,
...
class ClaudePtyManager {
  constructor() {
    ...
    this.maxInstances = Number.parseInt(process.env.CLAUDE_CHANNEL_MAX_SESSIONS || '2', 10);
    this.idleMs = Math.max(
      Number.parseInt(process.env.CLAUDE_CHANNEL_IDLE_MS || String(30 * 60 * 1000), 10),
      10 * 60 * 1000
    );
    ...
    this.conversationActiveChecker = null;
  }
  ...
  setConversationActiveChecker(checker) {
    this.conversationActiveChecker = typeof checker === 'function' ? checker : null;
  }

  hasActiveWebSocket(conversationId) {
    try {
      return Boolean(this.conversationActiveChecker?.(conversationId));
    } catch (error) {
      logger.warn('active WebSocket checker failed', { conversationId, error: error.message });
      return false;
    }
  }
  ...
  async ensureInstance(conversationId, options = {}) {
    ...
    const started = await this.ensureTmux(instance, options);
    await this.waitForChannel(instance, { started });
    return instance;
  }
  ...
  processConversationId(processInfo) {
    try {
      const cwd = fs.readlinkSync(`/proc/${processInfo.pid}/cwd`);
      const prefix = `${WORKDIR_DIR}${path.sep}`;
      if (cwd.startsWith(prefix)) {
        return safeConversationId(cwd.slice(prefix.length).split(path.sep)[0]);
      }
    } catch {
      // 进程刚退出时 /proc 可能已经消失，忽略即可。
    }
    return 'unknown';
  }

  duplicateProcessGroups(processes) {
    const groups = new Map();
    for (const item of processes || []) {
      const conversationId = this.processConversationId(item);
    ...
    return Array.from(groups.entries())
      ...
      .map(([conversationId, items]) => ({ conversationId, items }));
  }
  ...
  writeMcpConfig(instance) {
    ...
    // 自动化会话只能读取用户刚上传的附件目录，避免图片/PDF 卡在 Claude Code 的人工确认框。
    allow.add('Read(/var/lib/companion/uploads/**)');
  ...
  async ensureTmux(instance, options = {}) {
    if (await this.tmuxHasSession(instance)) return false;

    const permissionMode = config.claude.permissionMode || 'default';
    const args = [
      '--dangerously-load-development-channels', 'server:companion-channel',
      '--permission-mode', permissionMode,
      '--add-dir', config.uploadDir,
      '--allowedTools', 'Read,mcp__companion-channel__reply'
    ];
    // 固定 Bridge 主通道模型，避免 opus alias 解析到隐藏 thinking 的新版模型。
    const model = FIXED_CLAUDE_MODEL;
  ...
  async waitForChannel(instance, options = {}) {
    ...
    while (Date.now() - started < 45_000) {
      try {
        await getJson(url);
        if (options.started) await this.waitForTerminalReady(instance);
        return;
      } catch (error) {
        lastError = error;
  ...
  async waitForTerminalReady(instance) {
    const started = Date.now();
    let pane = '';
    while (Date.now() - started < 12_000) {
      ...
      try {
        ...
        logger.warn('Claude terminal ready check failed', { conversationId: instance.conversationId, error: error.message });
      }
      if (pane.includes('Listening for channel messages')) {
        await sleep(Number.parseInt(process.env.CLAUDE_CHANNEL_STARTUP_GRACE_MS || '1800', 10));
        logger.info('Claude terminal channel ready', {
          conversationId: instance.conversationId,
          tmuxSession: instance.tmuxSession,
    ...
    logger.warn('Claude terminal ready marker not observed; continuing after grace period', {
      conversationId: instance.conversationId,
      tmuxSession: instance.tmuxSession,
      waited_ms: Date.now() - started,
      pane_tail: pane.slice(-240)
    });
    await sleep(1000);
  }
  ...
  async watchdog() {
    ...
    const duplicateGroups = this.duplicateProcessGroups(processes);
    if (duplicateGroups.length) {
      const target = duplicateGroups[0];
      logger.warn('watchdog 发现同一 conversation 存在多个 Claude channel 进程，自动收敛', {
        conversationId: target.conversationId,
        count: target.items.length,
        pids: target.items.map((item) => item.pid)
      });
      await this.restartClaude({ reason: 'watchdog_duplicate_processes', conversationId: target.conversationId });
      return;
    }
  ...
  drain(instance) {
    ...
    this.startTranscriptWatcher(instance);
    ...
    job.handlers.onEvent?.({
      type: 'thinking',
      content: '',
      meta: {
        rawType: 'transcript_thinking_placeholder',
        hidden: true,
        placeholder: true,
        conversation_id: instance.conversationId,
        session_id: job.options.sessionId || 'default',
        claude_session_id: instance.active.claudeSessionId || null
      }
    });
  ...
  startTranscriptWatcher(instance) {
    this.stopTranscriptWatcher(instance);
    instance.transcriptPath = transcriptPathForSession(instance.workdir, instance.active?.claudeSessionId);
    instance.transcriptOffset = this.safeTranscriptSize(instance.transcriptPath);
    instance.transcriptTimer = setInterval(() => {
      this.pollTranscriptThinking(instance).catch((error) => {
        logger.warn('transcript thinking poll failed', {
          conversationId: instance.conversationId,
          error: error.message
        });
      });
    }, 600);
    instance.transcriptTimer.unref?.();
  }

  stopTranscriptWatcher(instance) {
    if (instance.transcriptTimer) {
      clearInterval(instance.transcriptTimer);
      instance.transcriptTimer = null;
    }
  }

  safeTranscriptSize(filePath) {
  ...
  async pollTranscriptThinking(instance) {
    if (!instance.active) return;
    let sessionId = instance.active.claudeSessionId || instance.claudeSessionId || null;
    if (!sessionId) {
      sessionId = findLatestClaudeSessionId(instance.workdir);
      if (sessionId) {
        instance.active.claudeSessionId = sessionId;
        instance.claudeSessionId = sessionId;
      }
    }
    const filePath = transcriptPathForSession(instance.workdir, sessionId);
    if (!filePath || !fs.existsSync(filePath)) return;
    if (instance.transcriptPath !== filePath) {
      instance.transcriptPath = filePath;
      instance.transcriptOffset = 0;
    }

    const stat = fs.statSync(filePath);
    if (stat.size < instance.transcriptOffset) {
      instance.transcriptOffset = 0;
    }
    if (stat.size === instance.transcriptOffset) return;

    const stream = fs.createReadStream(filePath, {
      encoding: 'utf8',
      start: instance.transcriptOffset,
      end: stat.size - 1
    });
    let chunk = '';
    for await (const part of stream) chunk += part;
    instance.transcriptOffset = stat.size;

    for (const line of chunk.split(/\r?\n/)) {
      if (!line.trim()) continue;
      const thoughts = extractThinkingFromTranscriptLine(line);
      for (const thought of thoughts) {
        const thoughtAt = thought.timestamp ? Date.parse(thought.timestamp) : Date.now();
        if (Number.isFinite(thoughtAt) && thoughtAt < instance.active.startedAt - 2000) continue;
        const key = `${thought.messageId || ''}:${thought.uuid || ''}:${thought.text || thought.signature || 'hidden'}`;
        if (instance.active.thinkingSeenKeys.has(key)) continue;
        instance.active.thinkingSeenKeys.add(key);
        const text = thought.text || '';
        if (text) instance.active.thinking.push(text);
        instance.active.handlers.onEvent?.({
          type: 'thinking',
          content: text,
          meta: {
            rawType: 'transcript_thinking',
            hidden: thought.hidden,
            placeholder: thought.placeholder,
            conversation_id: instance.conversationId,
            session_id: instance.active.options.sessionId || 'default',
  ...
  handleChannelReply(payload) {
    ...
    if (!segments.length) return { ok: false, reason: 'empty_segments' };
    ...
    instance.active.raw.push({ type: 'reply', text: cleanText, originalText: text, segments, payload });

    let delay = instance.active.eventDelayMs || 0;
    const hadPriorText = Boolean(instance.active.text.replace(cleanText, '').trim());
    segments.forEach((segment, index) => {
      const needsTypingPause = hadPriorText || index > 0 || delay > 0;
      if (needsTypingPause) {
        ...
        instance.active.scheduledTimers.push(typingTimer);
      ...
      instance.active.scheduledTimers.push(textTimer);
    });
    instance.active.eventDelayMs = delay;
    ...
    instance.active.replyTimer = setTimeout(() => {
      instance.active?.handlers.onEvent?.({
        ...
        content: '',
        meta: {
          rawType: 'transcript_thinking_complete',
          conversation_id: conversationId,
          session_id: instance.active.options.sessionId || 'default',
          claude_session_id: instance.active.claudeSessionId || null
        }
    ...
    }, (instance.active.eventDelayMs || 0) + this.replyQuietMs);
    return { ok: true };
  }

  handlePermissionRequest(payload) {
    logger.warn('Claude permission request via channel', payload);
    if (isUploadReadPermission(payload)) {
      this.approveUploadReadPermission(payload).catch((error) => {
        logger.error('自动批准上传附件读取失败', {
          conversationId: payload.conversation_id,
          requestId: payload.request_id,
          error: error.message
        });
      });
      return { ok: true, autoApproved: true, reason: 'upload_read' };
    }
    return { ok: true };
  }

  async approveUploadReadPermission(payload) {
    const conversationId = safeConversationId(payload.conversation_id || 'default');
    const instance = this.instances.get(conversationId);
    const tmuxSession = instance?.tmuxSession || tmuxSessionName(conversationId);
    await execFileP('tmux', ['-S', TMUX_SOCKET, 'send-keys', '-t', tmuxSession, '1', 'Enter'], { timeout: 5000 });
    logger.info('已自动批准上传附件 Read 权限', {
      conversationId,
      tmuxSession,
      requestId: payload.request_id
    });
  ...
  finishActive(instance, error, result = null) {
    ...
    this.stopTranscriptWatcher(instance);
    for (const timer of active.scheduledTimers || []) clearTimeout(timer);
    instance.lastUsedAt = Date.now();
  ...
  scheduleIdleKill(instance) {
    clearTimeout(instance.idleTimer);
    instance.idleTimer = setTimeout(() => {
      if (this.hasActiveWebSocket(instance.conversationId)) {
        logger.info('idle_timeout skipped because WebSocket is still active', {
          conversationId: instance.conversationId,
          tmuxSession: instance.tmuxSession,
          idleMs: this.idleMs
        });
